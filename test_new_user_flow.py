...
def test_new_user_flow():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        print(
            f"  Agent settings file: {Path(temp_dir) / locations.AGENT_SETTINGS_PATH}"
        )
        print(
            f"  File exists: "
            f"{(Path(temp_dir) / locations.AGENT_SETTINGS_PATH).exists()}"
        )
        ...
        dummy_agent_data = (
            '{"llm": {"model": "openai/gpt-4o-mini", "api_key": "test-key", '
            '"base_url": null, "usage_id": "agent"}, "tools": [], "mcp_config": {}, '
            '"agent_context": {"skills": [], "system_message_suffix": ""}, '
            '"condenser": null}'
        )
